...
void object(double dx, double dy, double dz) {
   glPushMatrix();
   ...
   /*
   double x0 = cos(th*M_PI/180);
   double y0 = 0;
   double z0 = sin(th*M_PI/180);

   double x1 = 0;
   double y1 = cos(-ph*M_PI/180);
   double z1 = sin(-ph*M_PI/180);

   double x2 = sin(th*M_PI/180);
   double y2 = sin(th*M_PI/180);
   double z2 = sin(th*M_PI/180);
   ...
   /*
   double magnitude = pow(pow(dx, 2) + pow(dy, 2) + pow(dz, 2), .5);
   double v[3] = {
   ...
   double xang = atan(dy/dz);
   
   double zang = atan(dx/dy);
   //printf("%f, %f, %f", xang, yang, zang);
   ...
   //glRotatef(zang, 0, 0, 1);
   if(dx >= 0) {
      double yang = atan(dz/dx);
      yang *= 180/M_PI;
      glRotatef(-yang, 0, 1, 0);
   } else {
      double yang = atan(dx/dz);
      yang *= 180/M_PI;
      if(z <= 0)
         yang += 90;
      else
 ...
 */
void key(unsigned char ch,int dx,int dy)
{
   //  Exit on ESC
   if (ch == 27)
      exit(0);
